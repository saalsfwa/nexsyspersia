class NeuralNetwork {
  ...
  animate() {
    ...
    this.nodes.forEach(node => {
      ...
      const zoomFactor = 1 + (this.scrollY / 5000);
      ...
      this.ctx.fillStyle = `rgba(0, 212, 255, ${0.3 * scale})`;
      this.ctx.fill();
    });
    ...
    this.nodes.forEach((node1, i) => {
      this.nodes.slice(i + 1).forEach(node2 => {
        ...
        if (distance < 150) {
          ...
          const opacity = (1 - distance / 150) * 0.2 * scale;
